...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      /*
        读取 objectWrapperFactory 对象包装工厂, 包装实现：
      ...
      // 读取反射工厂 --> 默认实现类 DefaultReflectorFactory
      reflectorFactoryElement(root.evalNode("reflectorFactory"));

      // 把其他的配置信息 set 到 configuration 对象中
      settingsElement(settings);

      // read it after objectFactory and objectWrapperFactory issue #631
      // 在 objectFactory 和 objectWrapperFactory 问题 631 之后阅读它
      // 读取环境配置  在开发中常用的配置 主要是 事务管理 和 数据源 的配置
      environmentsElement(root.evalNode("environments"));

      // 读取数据库厂商标识配置信息
      databaseIdProviderElement(root.evalNode("databaseIdProvider"));

      // 读取数据类型处理器 typeHandlers
  ...
  /**
   * 将配置注入到 configuration 对象中
   * 每个参数具体的含义到官网(https://mybatis.org/mybatis-3/zh/configuration.html#settings)查看
   * @param props
   */
    ...
    if (parent != null) {
      for (XNode child : parent.getChildren()) {
        ...
        if ("package".equals(child.getName())) {
          ...
          // 扫描包 将映射注册到 configuration 中
          configuration.addMappers(mapperPackage);
        } else {
          // 其他的 resource、url、class 配置
